...
par(mfrow=c(1,1))
pbi <- txtProgressBar(min = 1, max = length(sitelev), style = 3)
  ...
  for (k in 1:length(eventlev)) {
    ...
    for(i in 3:3){
      ...
      if(i == 3){
        ...
        plot(temp.d$Us,temp.d$SF_gs, main = paste(sitelev[j],"_",eventlev[k],sep="") ,
             xlab = "wind friction(m/s)", ylab = "saltation flux (g/s m^2)",
             xlim = c(0,xmax), ylim = c(0,ymax))

      }else{
        par(new = T)
        plot(temp.d$Us,temp.d$SF_gs, xlab = "", ylab = "",col = i,
             xlim = c(0,xmax), ylim = c(0,ymax))
      }
    }
    if(boolplot){
